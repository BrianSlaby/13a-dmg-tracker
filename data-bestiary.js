...

{
    ...
    hp: 45,
    health: 45,
    attacks: ["Caustic Bite +8 vs AC (one creature wearing light armor or no armor) - 10 damage, and 5 ongoing acid damage. Natural 16+: The target is now considered to be wearing no armor (the armor is damaged beyond repair; adjust defenses accordingly). If the target's armor is magical, the target must roll a hard save. On a success, the armor isn't affected. This effect is permanent until the creature obtains new armor.", "Rusting Antenna +8 vs PD (one creature wearing heavy armor) - 1 damage, and the target is now considered to be wearing no armor (the armor is damaged beyond repair; adjust defenses accordingly). If the target's armor is magical, the target must roll a hard save. On a success, the armor isn't affected. This effect is permanent until the creature obtains new armor."],
    abilities: ["Corrupting Body: Whenever a creature hits the rust monster with a melee weapon, the attacker must roll a save; on a failure, the weapon is destroyed. Magic weapons require an easy save instead. This effect is permanent.", "Rust's Targets: Magic items wielded by creatures with 60 HP or more are not affected by the rust monster's ability to destroy items (but the target still takes damage).", "Saving Quirks: Before rolling a save for a magic item, a PC can gain a +5 bonus to the save by agreeing to roleplay the item's quirk as a huge element of their personality until the next full heal up. Failure to live up to this roleplaying agreement means that the item hasn't received the support it needs from its owner to survive the rust monster's destructive effects, and the item is destroyed at the end of the next battle even if its owner successfully saved.", "[Nastier Special] Big Antenna: When the rust monster scores a critical hit, it can try to destroy another random magic item in the target's possession (or a random normal item if the target has no other magic items). The creature must roll a save for the item, as usual."],
    isStaggered: false,
    id: 291
},

{
    ...
    hp: 72,
    health: 72,
    attacks: ["Caustic Bite +10 vs AC (one creature wearing light armor or no armor) - 18 damage, and 5 ongoing acid damage. Natural 14+: The target is now considered to be wearing no armor (the armor is damaged beyond repair; adjust defenses accordingly). If the target's armor is magical, the target must roll a hard save. On a success, the armor isn't affected. This effect is permanent until the creature obtains new armor.", "Rusting Antenna +10 vs PD (one creature wearing heavy armor) - 10 damage, and the target is now considered to be wearing no armor (the armor is damaged beyond repair; adjust defenses accordingly). If the target's armor is magical, the target must roll a hard save. On a success, the armor isn't affected. This effect is permanent until the creature obtains new armor. Each time the rust monster obliterator destroys heavy armor with this attack, it gains a different random demon ability until the end of the battle."],
    abilities: ["Corrupting Body: Whenever a creature hits the rust monster with a melee weapon, the attacker must roll a save; on a failure, the weapon is destroyed. Magic weapons require an easy save instead. This effect is permanent.", "Rust's Targets: Magic items wielded by creatures with 100 HP or more are not affected by the rust monster's ability to destroy items (but the target still takes damage).", "Saving Quirks: Before rolling a save for a magic item, a PC can gain a +5 bonus to the save by agreeing to roleplay the item's quirk as a huge element of their personality until the next full heal up. Failure to live up to this roleplaying agreement means that the item hasn't received the support it needs from its owner to survive the rust monster's destructive effects, and the item is destroyed at the end of the next battle even if its owner successfully saved.", "[Nastier Special] Tail Whirligig: When an enemy targets the rust monster obliterator with a spell, the rust monster rolls a save. On a success, the spell has no effect on the rust monster. If the rust monster is Staggered, it must roll a hard save instead."],
    isStaggered: false,
    id: 292
},
...

{
    ...
    hp: 34,
    health: 34,
    attacks: ["Coral-tipped Spear +6 vs AC - 6 damage. Natural Even Hit Against Stuck or Hampered Enemy: The target takes 1D6 extra damage.", "R: Hooked Net +6 vs PD - 3 damage. Natural Even Hit: The target is Hampered (save ends). Natural Odd Hit: The target is Stuck (save ends). Limited Use: 1/battle."],
    abilities: ["Blood Rage: The sahuagin raider gains a +2 attack bonus while it's Staggered.", "Water-breather: Sahuagin breathe underwater and swim very well."],
    isStaggered: false,
    id: 293
},

{
    ...
    hp: 10,
    health: 10,
    attacks: ["Razortoothed Jaw +7 vs AC - 4 damage. Natural Even Hit: Increase the damage by +1 for each razor shark mook that has dropped this battle. Natural Even Miss: The target and the razor shark each take 1D6 damage."],
    abilities: ["Frenzy (group): When two or more razor sharks in the same mob are dropped to 0 HP by an attack, as a free action each dropped razor shark attacks one random nearby non-razor shark creature before it dies. That creature takes 1D8 damage.", "Water-breather: Sharks breathe underwater and swim very well. They're not so good out of sea water (even if they jump), so treat them as semi-hazardous terrain while they're busy thrashing to death outside the water."],
    isStaggered: false,
    id: 294
},

{
    ...
    hp: 70,
    health: 70,
    attacks: ["Coral Rod +10 vs AC - 12 damage. Natural Even Hit or Miss: The priest teleports the target next to one of its nearby allies that it can see, who engages it as a free action. It can't teleport the target to a location that causes it direct damage (so into a pool of flaming oil is out, but off a ship into the sea is fine).", "R: Glowpriest's Prayer +10 vs MD (1D3 nearby enemies) - 12 psychic damage, and if the attack hits 2 or more targets, roll 1D8 after the attack for the prayer's effect. 1-2 (Hymn of Hate): One random nearby conscious enemy takes 4D6 negative energy damage. 3-4 (Curse of Despair): Each target that was hit takes a -2 penalty (non-cumulative) to all saves until the end of the battle. 5-6 (Scream of Victory): Until the start of the glowpriest's next turn, each sahuagin and demon in the battle adds the escalation die to its attacks and the PCs don't. GM: a story guide icon success with the Diabolist might let a PC get around this penalty. 7-8 (Word of Refuge): Remove the glowpriest from play. At the start of its next turn, return it to play nearby its original location. It gains a +4 bonus to all defenses until the end of its next turn after it returns to play."],
    abilities: ["Water-breather: Sahuagin breathe underwater and swim very well."],
    isStaggered: false,
    id: 295
},

{
    name: "Iron Sea Shark (double-strength mook)",
    level: 5,
    type: "mook",
    mookNumber: 1,
    initiative: "+6",
    ...
    hp: 32,
    health: 32,
    attacks: ["Massive Iron Jaws +10 vs AC - 14 damage. Miss: 2D6 damage."],
    abilities: ["Frenzy: While Staggered, if the iron sea shark is unengaged at the start of its turn, it must roll an easy save. On a failure, the shark must move and attack a random nearby enemy that's Staggered, or a random nearby creature if there are no Staggered enemies.", "Shredder: When an enemy misses with a melee attack against an iron sea shark and rolls a natural 1-5, the attacker takes 2D6 damage.", "Water Breather: Sharks breathe underwater and swim very well. They're not so good out of sea water (even if they jump), so treat them as semi-hazardous terrain while they're busy thrashing to death outside the water."],
    isStaggered: false,
    id: 296
},

{
    ...
    hp: 200,
    health: 200,
    attacks: ["Four-armed Frenzy +11 vs AC (1D4 attacks) - 15 damage. Natural 4, 8, 12, 16, 20: The target takes 10 ongoing damage.", "R: Heavy Crossbow +10 vs AC (one nearby or far away enemy) - 30 damage. Natural 16+: The target takes 10 ongoing damage."],
    abilities: ["Limited Escalator: The sahuagin mutant adds the escalation die to its attacks until the die is 5+.", "Water Breather: Sahuagin breathe underwater and swim very well.", "[Nastier Special] Punisher: When an enemy intercepts the sahuagin mutant, it takes 4D8 damage from the mutant's claws and teeth.", "[Nastier Special] Ripper: When an enemy makes a melee attack against the sahuagin mutant and rolls a natural odd miss, that enemy takes 2D8 damage."],
    isStaggered: false,
    id: 297
},
...

{
    ...
    hp: 9,
    health: 9,
    attacks: ["Spear, Sword, or Club +7 vs AC - 4 damage. Miss: 1 damage."],
    abilities: ["N/A"],
    isStaggered: false,
    id: 309
},

{
    ...
    hp: 86,
    health: 86,
    attacks: ["Whipping Hook +8 vs AC - 10 damage, and 10 ongoing damage. Miss: 5 damage.", "C: Crusader's Banner Magic: Roll 2D20 and use each roll (or a lower result of your choice) to determine one effect. 1-5: The warbanner's bearer can make a basic attack with a +2 attack bonus as a free action. 6-10: Each different enemy engaged with a creature in the warbanner's band takes 1D8 psychic damage. 11-15: Until the end of the battle, each ally in the warbanner's band gains a +2 cumulative bonus to damage on hits with melee attacks. 16-20: The warbanner can make a False Rally attack as a free action.", "[Special Trigger] C: False Rally +8 vs MD (one nearby enemy with 15 HP or fewer) - The target is Confused hard save ends. Critical Hit: The target can now only save against the confusion with a natural 20 (or until the warbanner drops) and can't flee if the rest of the PCs choose to do so."],
    abilities: ["Designated Bearer: Once per round as a quick action, the warbanner can fly directly above a lower-level ally in its band and designate that ally as its bearer. Alternately, it can release itself from a bearer and fly free.", "Bearer Beware: While the warbanner has a bearer it moves with the bearer and is engaged by enemies that engage the bearer. The bearer is Vulnerable to all attacks but it can use the escalation die.", "Conditional Resist Damage 16+: Unless an ally in the warbanner's band dropped to 0 HP since the start of the current round, an enemy attacking the warbanner must roll a natural 16+ on the attack roll or it only takes half damage. This ability has no effect if the warbanner has no allies in its band.", "Flight: Warbanners fly quickly, fluttering in winds only they can feel, preferably never touching the ground. They can't fly off the ground higher than a flag on a pole can be carried into battle, so enemies on the ground can still target them. At best, a warbanner might gain a +2 bonus to defenses against attacks made by small PCs like halflings and gnomes.", "Mook Morale: Mooks fighting in the warbanner's band gain a bonus to their hit points equal to the warbanner's level (3)."],
    isStaggered: false,
    id: 310
},

{
    ...
    hp: 100,
    health: 100,
    attacks: ["Flagspear +9 vs AC - 18 damage. Miss: 8 damage.", "C: Orc Lord's Banner Magic: Roll 2D20 and use each roll (or a lower result of your choice) to determine one effect. 1-5: The warbanner's bearer can make a basic attack with a +2 bonus as a free action. 6-10: Add a 3rd level orc mook to the warbanner's band (it's either healed by the warbanner or freshly spat out of the earth). 11-15: The warbanner can make a Flagspear attack as a free action. 16-20: The warbanner can make a False Rally attack as a free action.", "[Special Trigger] False Rally +8 vs MD (one nearby enemy with 15 HP or fewer) - The target is Confused (hard save ends). Critical Hit: The target can now only save against the confusion with a natural 20 (or until the warbanner drops) and can't flee if the rest of the PCs choose to do so."],
    abilities: ["Designated Bearer: Once per round as a quick action, the warbanner can fly directly above a lower-level ally in its band and designate that ally as its bearer. Alternately, it can release itself from a bearer and fly free.", "Bearer Beware: While the warbanner has a bearer it moves with the bearer and is engaged by enemies that engage the bearer. The bearer is Vulnerable to all attacks but it can use the escalation die.", "Conditional Resist Damage 16+: Unless an ally in the warbanner's band dropped to 0 HP since the start of the current round, an enemy attacking the warbanner must roll a natural 16+ on the attack roll or it only takes half damage. This ability has no effect if the warbanner has no allies in its band.", "Flight: Warbanners fly quickly, fluttering in winds only they can feel, preferably never touching the ground. They can't fly off the ground higher than a flag on a pole can be carried into battle, so enemies on the ground can still target them. At best, a warbanner might gain a +2 bonus to defenses against attacks made by small PCs like halflings and gnomes.", "Mook Morale: Mooks fighting in the warbanner's band gain a bonus to their hit points equal to the warbanner's level (3)."],
    isStaggered: false,
    id: 311
},

{
    ...
    hp: 104,
    health: 104,
    attacks: ["Bladed Fringe +8 vs AC - 15 damage, and 10 ongoing negative energy damage. Miss: 10 damage.", "C: Lich King's Banner Magic - Roll 2D20 and use each roll (or a lower result of your choice) to determine one effect. 1-5: The warbanner's bearer can make a basic attack with a +2 attack bonus as a free action. 6-10: Each different enemy engaged with a creature in the warbanner's band takes 1D10 psychic damage. 11-15: If any mooks in the warbanner's band have dropped this battle, return one of those mooks to the battle next to the warbanner. (If the warbanner has no mooks in its band, choose a lower result). 16-20: The warbanner can make a False Rally attack as a free action.", "[Special Trigger] C: False Rally +9 vs MD (one nearby enemy with 18 HP or fewer) - The target is Confused (hard save ends). Critical Hit: The target can now only save against the confusion with a natural 20 (or until the warbanner drops) and can't flee if the rest of the PCs choose to do so."],
    abilities: ["Designated Bearer: Once per round as a quick action, the warbanner can fly directly above a lower-level ally in its band and designate that ally as its bearer. Alternately, it can release itself from a bearer and fly free.", "Bearer Beware: While the warbanner has a bearer it moves with the bearer and is engaged by enemies that engage the bearer. The bearer is Vulnerable to all attacks but it can use the escalation die.", "Conditional Resist Damage 16+: Unless an ally in the warbanner's band dropped to 0 HP since the start of the current round, an enemy attacking the warbanner must roll a natural 16+ on the attack roll or it only takes half damage. This ability has no effect if the warbanner has no allies in its band.", "Flight: Warbanners fly quickly, fluttering in winds only they can feel, preferably never touching the ground. They can't fly off the ground higher than a flag on a pole can be carried into battle, so enemies on the ground can still target them. At best, a warbanner might gain a +2 bonus to defenses against attacks made by small PCs like halflings and gnomes.", "Mook Morale: Mooks fighting in the warbanner's band gain a bonus to their hit points equal to the warbanner's level (4)."],
    isStaggered: false,
    id: 312
},
...

{
    ...
    hp: 28,
    health: 28,
    attacks: ["Claws and Bite +6 vs AC (2 attacks) - 4 damage. Natural 16+: The hatchling can make an Ice Breath attack as a free action.", "[Special Trigger] C: Ice Breath +6 vs PD - 4 cold damage. Natural Odd Miss: The hatchling takes 1D3 damage."],
    abilities: ["Vulnerability: Fire", "Resist Cold 12+", "Escalator: A dragon adds the escalation die to its attack rolls.", "Flight: A dragon flies reasonably well, powerful and fast in a straight line though not as maneuverable as more agile fliers.", "Freezing Vapors: When the hatchling scores a critical hit, it prevents the escalation die from increasing at the start of the next round."],
    isStaggered: false,
    id: 317
},

{
    ...
    hp: 48,
    health: 48,
    attacks: ["Claws and Bite + 7 vs AC (2 attacks) - 6 damage. Natural 16+: The cenotaph dragon can make an Infused Ice Breath attack as a free action.", "[Special Trigger] C: Infused Ice Breath +7 vs PD (1D3 nearby enemies) - 6 cold damage. Natural 20: The target also takes 5 ongoing holy damage (in addition to the double damage for a crit). Natural Odd Hit or Miss: The dragon takes 1D6 damage."],
    abilities: ["Vulnerability: Fire", "Resist Cold and Negative Energy 12+", "Escalator: A dragon adds the escalation die to its attack rolls.", "Flight: A dragon flies reasonably well, powerful and fast in a straight line though not as maneuverable as more agile fliers."],
    isStaggered: false,
    id: 318
},

{
    ...
    hp: 140,
    health: 140,
    attacks: ["Claws and Bite +9 vs AC (2 attacks) - 16 damage. Natural 16+: The mausoleum dragon can make an Infused Ice Breath attack as a free action.", "[Special Trigger] C: Infused Ice Breath +9 vs PD (1D3 nearby enemies) - 14 cold damage. Natural Even Hit: The target also takes 7 ongoing holy damage. Natural Odd Hit or Miss: The dragon takes 2D6 damage."],
    abilities: ["Vulnerability: Fire", "Resist Cold and Negative Energy 16+", "Escalator: A dragon adds the escalation die to its attack rolls.", "Flight: A dragon flies reasonably well, powerful and fast in a straight line though not as maneuverable as more agile fliers.", "Infused Blood: When an enemy scores a critical hit against teh mausoleum dragon, the dragon gains teh following ability until the end of the battle: when an enemy is engaged with the dragon at the start of its turn, it takes 5 holy damage."],
    isStaggered: false,
    id: 319
},

{
    ...
    hp: 175,
    health: 175,
    attacks: ["Claws and Bite +10 vs AC (2 attacks) - 18 damage. Natural 16+: The blizzard dragon can make a Blizzard Breath attack as a free action.", "[Special Trigger] C: Blizzard Breath +10 vs PD (each nearby creature) - 20 cold damage, and 5 ongoing cold damage. Natural 18+: The target takes 10 ongoing cold damage instead of 5. Natural Odd Hit or Miss: The dragon takes 2D6 damage."],
    abilities: ["Vulnerability: Fire", "Resist Cold 14+", "Escalator: A dragon adds the escalation die to its attack rolls.", "Flight: A dragon flies reasonably well, powerful and fast in a straight line though not as maneuverable as more agile fliers.", "Continuous Breath: A blizzard dragon can use Blizzard Breath as its standard action during the round after it uses the breath as a free action. Blizzard Breath doesn't deal miss damage to the dragon when used as a standard action.", "Wreathed in Snow and Cold: When an enemy is engaged with the blizzard dragon at the start of its turn, it takes 5 cold damage."],
    isStaggered: false,
    id: 320
},

{
    ...
    hp: 324,
    health: 324,
    attacks: ["Claws and Bite +12 vs AC (2 attacks) - 40 damage. Natural 16+: The moon dragon can make an Ice Breath attack as a free action. Both Attacks Miss: If the full moon is within seven days, or if the battle is in Moonwreck, the moon dragon's next Ice Breath attack this battle will deal double damage.", "[Special Trigger] C: Ice Breath +12 vs PD (1D3 nearby enemies) - 32 cold damage."],
    abilities: ["Resist Cold 18+", "Escalator: A dragon adds the escalation die to its attack rolls.", "Flight: A dragon flies reasonably well, powerful and fast in a straight line though not as maneuverable as more agile fliers.", "Lunar Resonance: A moon dragon gains a +1 bonus to attacks when the full moon is within seven days. A moon dragon takes a -1 penalty to all defenses and gains Vulnerability: Fire when the new moon is within seven days. For moon dragons in Moonwreck, the power of the full moon is always with them, so many of them lair there."],
    isStaggered: false,
    id: 321
},
